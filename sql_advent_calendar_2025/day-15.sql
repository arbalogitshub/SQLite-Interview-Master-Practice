...
SELECT 
  log_date AS current_day,
  mischief_score AS current_mischief_score,
  LAG(mischief_score, 1) OVER (ORDER BY log_date) 
    AS previous_day_score,
   mischief_score -  LAG(mischief_score, 1) OVER (ORDER BY log_date)
    AS mischief_score_point_difference
